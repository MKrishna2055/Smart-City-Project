...
# ESP32 devices use this shared key when posting readings. Set SENSOR_API_KEY
# in the deployment environment instead of committing a real key to Git.
...
def home():
    ...
    # These values come from the real Leaflet/OpenStreetMap dashboard map.
    # They are the only location source used by the live dashboard cards.
    map_lat = request.args.get("map_lat")
    map_lng = request.args.get("map_lng")
    selected_location_name = request.args.get("location") or "Select a real map location"

    status = city_status(selected, map_lat, map_lng, selected_location_name)
    waste = {}
    current_item, correct_bin = random.choice(WASTE_ITEMS)

    message = request.args.get("message", "")
    water_form = {
        "ph": request.args.get("ph", "7.2"),
        "oxygen": request.args.get("oxygen", "6.5"),
        "turbidity": request.args.get("turbidity", "35"),
    }
...
def init_db():
    ...
    with db() as conn:
        ...
        # Stores the latest and historical readings received from physical
        # ESP32 devices. This is separate from manually entered water samples.
        conn.execute(
            """
        ...
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_esp32_readings_created_at
            ON esp32_readings (created_at DESC)
            """
        )
        conn.commit()
...
def city_status(selected=None, map_lat=None, map_lng=None, location_name="City dashboard"):
    """Create one full random dashboard reading cycle synced to a location."""
    # Real world map coordinates influence the simulated readings after a search/click.
    # This keeps the dashboard synced to Leaflet/OpenStreetMap locations.
    coordinate_factor = None
    if map_lat and map_lng:
        try:
            coordinate_factor = int((abs(float(map_lat)) * 7 + abs(float(map_lng)) * 5) % 100)
        except ValueError:
            coordinate_factor = None

    if coordinate_factor is not None:
        aqi = max(22, min(248, coordinate_factor * 2 + random.randint(0, 48)))
    elif selected:
        aqi = max(22, min(248, selected["pollution"] * 2 + random.randint(-8, 18)))
    else:
    ...
    # These values feed the location-synced dashboard cards.
    algae_risk = random.choice(["Low", "Moderate", "High"])
    if coordinate_factor is not None:
        water_quality = max(48, min(98, 110 - coordinate_factor + random.randint(-8, 10)))
    elif selected:
        water_quality = max(48, min(98, 100 - selected["pollution"] + random.randint(-6, 12)))
    else:
    ...
    # Temperature is treated as another live dashboard parameter.
    # The weather label changes with the current time so the dashboard shows day/night context.
    hour = datetime.now().hour
    is_daytime = 6 <= hour < 18
    if not is_daytime:
        weather_condition = random.choice(["Night", "Cloudy night", "Clear night"])
        temperature = random.randint(17, 27)
    else:
        weather_condition = random.choice(["Sunny", "Cloudy", "Rainy", "Snowy"])
        temperature = random.randint(-2, 38) if weather_condition == "Snowy" else random.randint(22, 39)
    ...
    return {
        ...
        "air_class": air_class,
        "air_advice": advice,
        "traffic": max(24, min(98, coordinate_factor + random.randint(10, 28))) if coordinate_factor is not None else selected["footfall"] if selected else random.randint(58, 94),
        "temperature": temperature,
        "weather_condition": weather_condition,
        "daylight": "Day" if is_daytime else "Night",
        ...
        "solar": random.randint(56, 96) if is_daytime else random.randint(0, 12),
    }
...
def latest_esp32_reading():
    """Return the newest BMP280 reading received from any ESP32 device."""
    with db() as conn:
        row = conn.execute(
            "SELECT * FROM esp32_readings ORDER BY id DESC LIMIT 1"
        ).fetchone()
    return dict(row) if row else None


@app.post("/api/esp32/readings")
def receive_esp32_reading():
    """Accept one authenticated BMP280 reading from an ESP32."""
    supplied_key = request.headers.get("X-Sensor-Key", "")
    if not SENSOR_API_KEY or not hmac.compare_digest(supplied_key, SENSOR_API_KEY):
        return jsonify({"error": "Unauthorized sensor"}), 401
    ...
    try:
        ...
        return jsonify({"error": "temperature and pressure must be numeric"}), 400

    # BMP280's usable temperature range is roughly -40 to 85 C. The pressure
    # bounds prevent corrupt or accidental values from being shown on the UI.
    if not (-40 <= temperature <= 85 and 300 <= pressure <= 1100):
        return jsonify({"error": "Reading is outside the accepted BMP280 range"}), 422

    device_id = str(payload.get("device_id", "esp32-bmp280"))[:64]
    location = str(payload.get("location", "ESP32 sensor"))[:120]
    created_at = datetime.now().strftime("%d %b %Y, %I:%M %p")

    with db() as conn:
        conn.execute(
            """
            INSERT INTO esp32_readings (device_id, location, temperature, pressure, created_at)
            VALUES (?, ?, ?, ?, ?)
            """,
            (device_id, location, temperature, pressure, created_at),
        )
        conn.commit()
...
def get_latest_esp32_reading():
    """Expose the latest stored sensor reading to the dashboard UI."""
    reading = latest_esp32_reading()
    if reading is None:
        return jsonify({"connected": False, "reading": None})
    return jsonify({"connected": True, "reading": reading})
...
def water_quality():
    ...
    location = request.form.get("location", "Selected real map location")
    map_lat = request.form.get("map_lat", "")
    map_lng = request.form.get("map_lng", "")
    back_url = url_for(
        "home",
        location=location,
        map_lat=map_lat,
        map_lng=map_lng,
        ph=request.form.get("ph", "7.2"),
        oxygen=request.form.get("oxygen", "6.5"),
        turbidity=request.form.get("turbidity", "35"),
    ) + "#water-quality"
